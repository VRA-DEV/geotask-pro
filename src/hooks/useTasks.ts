...
function buildUrl(opts: UseTasksOptions = {}): string {
  ...
  if (opts.responsibleId) params.set("responsible_id", String(opts.responsibleId));
...
export function useTasks(opts: UseTasksOptions = {}) {
  ...
  const tasks: Task[] = isPaginated ? (data?.data || []) : (data || []);
  const pagination = isPaginated ? data?.pagination : null;
